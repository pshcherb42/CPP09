...
void insert_pend_using_jacobsthal(std::vector<int>& main_seq, std::vector<int>& pend, int block_len) {
	...
	while(!pend.empty()) {
		...
		std::cout << "curr_jac: " << curr_jac << "\n";
		while (idx) {
			std::cout << "idx dentro de loop: " << idx << std::endl;
			int high = std::min(static_cast<int>(max_values.size()), static_cast<int>(_jacobsthal_number(curr_jac - 1)));
			high -= count;
			std::cout << "high: " << high << "\n";
			loc = binary_search(max_values, pend[block_len * idx - 1], 0, high);
